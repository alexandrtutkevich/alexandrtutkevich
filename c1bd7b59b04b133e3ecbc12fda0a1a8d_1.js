    ...
    event.waitUntil(
        self.registration.pushManager.getSubscription()
            .then(function (subscription) {
                //Для монетизации пушей
                if (isPushPush && taskIdNew) {
                    var aNotification = {};
                    var aTaskId = taskIdNew;
                    if (typeof aTaskId === 'string') {
                        aTaskId = JSON.parse(aTaskId);
                    }
                    return fetch(pushPushUrl, {
                        method: 'post',
                        body: JSON.stringify({
                            'task_info': aTaskId
                        })
                        ...
                        aNotification['body'] = value.body;
                        aNotification['icon'] = value.icon;
                        aNotification['data'] = {};
                        aNotification['data']['redirectUrl'] = value.link;
                        aNotification['image'] = value.image;
                        return self.registration.showNotification(value.title, aNotification);
                    ...
                    if (oSentData.message) {
                        ...
                        self.registration.pushManager.getSubscription().then(function (subscription) {
                            ...
                            var aSubscriptionId = subscriptionId.split('/');
                            subscriptionId = aSubscriptionId[(aSubscriptionId.length - 1)]

                            var title = Base64.decode(oSentData.message.title);
                            var message = Base64.decode(oSentData.message.message);
                            var icon = oSentData.message.icon;
                            var notificationTag = oSentData.message.tag;
                            var url = Base64.decode(oSentData.message.url);
                            ...
                            if (oSentData.message.buttons !== undefined) {
                                aNotification['actions'] = oSentData.message.buttons;
                                aNotification['data']['buttons'] = oSentData.message.buttons;
                            }
                            if (oSentData.message.image !== undefined) {
                                aNotification['image'] = oSentData.message.image;
                            }
                            ...
                            var statObj = {
                                ...
                                task_id: oSentData.message.task_id,
                                user_id: oSentData.message.user_id
                            };

                            if (oSentData.message.addData !== undefined) {
                                statObj['addData'] = oSentData.message.addData;
                            }
                            ...
                            return fetch(sServerApi, {
                                ...
                                return self.registration.showNotification(title, aNotification);
                            }).catch(function (err) {
                                return self.registration.showNotification(title, aNotification);
                            });
                        })
                        ...
                        if (oSentData.notification) {
                            var title = oSentData.notification.title;
                            var message = oSentData.notification.body;
                            var icon = oSentData.notification.icon;
                        } else {
                            var title = undefined;
                        ...
                        if (title === undefined) {
                            var title = oSentData.data.title;
                        }
                        if (message === undefined) {
                            var message = oSentData.data.body;
                        }
                        if (icon === undefined) {
                            var icon = oSentData.data.icon;
                        }
                        var notificationTag = oSentData.collapse_key;
                        var url = oSentData.data.url;
                        var subscriptionId = oSentData.data.subscription;
                        var requireInteraction = (oSentData.data.requireInteraction == "true") ? true : false;
                        var iTaskId = parseInt(oSentData.data.task_id);
                        var iUserId = parseInt(oSentData.data.user_id);
                        ...
                        if (oSentData.data.buttons !== undefined) {
                            if (typeof oSentData.data.buttons === 'string') {
                                var oButtons = JSON.parse(oSentData.data.buttons);
                            } else {
                                var oButtons = oSentData.data.buttons;
                            }
                            aNotification['actions'] = oButtons;
                            aNotification['data']['buttons'] = oButtons;
                        }
                        if (oSentData.data.image !== undefined) {
                            aNotification['image'] = oSentData.data.image;
                        }
                        ...
                        if (oSentData.data.addData !== undefined) {
                            statObj['addData'] = oSentData.data.addData;
                        }
...
self.addEventListener('notificationclick', function (event) {
    var redirectUrl = event.notification.data.redirectUrl;

    if (event.action !== undefined) {
        var action = event.action;
        if (event.notification.data.buttons !== undefined) {
            for (var i = 0; i < event.notification.data.buttons.length; i++) {
                if (event.notification.data.buttons[i].action == action) {
                    redirectUrl = event.notification.data.buttons[i].url;
                }
            }
        }
    }
